...
Set-Alias SQLCompare 'C:\Program Files (x86)\Red Gate\SQL Compare 10\SQLCompare.exe' -Scope Script
Set-Alias SQLDataCompare 'C:\Program Files (x86)\Red Gate\SQL Data Compare 10\SQLDataCompare.exe' -Scope Script
...
# if there is no build scripts there, then generate them from build database on the current build server 
...
$AllArgs = @("/scripts1:$($data.source.DatabasePath)/Source", "/server2:$($data.build.NewBuildServerInstance)",
    '/quiet', "/exclude:user",'/options:ThrowOnFileParseFailed,IgnoreCollations,IgnoreSchemaObjectAuthorization',
...
if ($ExistingDatabase -ne $null)
{
    ...
    {Throw "Could not delete $($data.build.NewDatabase) on $($server.Name) because it still has users."}
}
...
$data.clones | foreach {
    ...
    if (($clone) -ne $null)
    {
        write-warning  "Removing Clone $($_.Database) that already existed on $($_.NetName)"
